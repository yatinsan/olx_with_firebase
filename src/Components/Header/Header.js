...
useEffect(() => {
  console.log(user)
  // Firebase.firestore().collection('postes').where('name','==',search).get().then((snap) =>{snap.docs.map((doc) =>{console.log(doc.data())})})
  Firebase.firestore().collection('postes').where('name','array-contains-any',[search]).get().then((snap) =>{setResult(snap.docs.map((doc) =>doc.data()))})
  // Firebase.firestore().collection('postes')
  result[0] && console.log(result[0].name)
      ...
      <div className="headerChildDiv">
        ...
        <div className="productSearch">
          ...
          <div className="input d-flex flex-column ">
            ...
            {/* </div> */}
            { result &&
            <div class="cardsearch gggk">
             <ul class="list-group list-group-flush resultbox">
                {result.map((res)=> <li class="list-group-item gggk">{res.name}</li>)}
